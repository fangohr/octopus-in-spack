...
RUN . $SPACK_ROOT/share/spack/setup-env.sh && \
      ...
      spack spec octopus@${OCT_VERSION} ~mpi+netcdf+parmetis+arpack+cgal+pfft+python+likwid+libyaml+elpa+nlopt && \
      # run the spack installation (adding it to the environment):
      spack add octopus@${OCT_VERSION} ~mpi+netcdf+parmetis+arpack+cgal+pfft+python+likwid+libyaml+elpa+nlopt && \
      spack install && \
