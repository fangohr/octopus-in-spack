...
RUN . $SPACK_ROOT/share/spack/setup-env.sh && \
      ...
      spack spec octopus@${OCT_VERSION} ~mpi+netcdf+arpack+cgal+python+likwid+libyaml+elpa+nlopt+berkeleygw~debug~cuda~metis && \
      # run the spack installation (adding it to the environment):
      spack add octopus@${OCT_VERSION} ~mpi+netcdf+arpack+cgal+python+likwid+libyaml+elpa+nlopt+berkeleygw~debug~cuda~metis && \
      spack install && \
...
RUN . $SPACK_ROOT/share/spack/setup-env.sh && \
      ...
      # display specs of upcoming spack installation:
      spack spec octopus@${OCT_VERSION} +mpi +netcdf+parmetis+arpack+cgal+pfft+python+likwid+libyaml+elpa+nlopt+berkeleygw~debug~cuda~metis~scalapack  && \
      # run the spack installation (adding it to the environment):
      spack add octopus@${OCT_VERSION} +mpi +netcdf+parmetis+arpack+cgal+pfft+python+likwid+libyaml+elpa+nlopt+berkeleygw~debug~cuda~metis~scalapack  && \
      spack install && \
      # run spack smoke tests for octopus. We get an error if any of the fails:
